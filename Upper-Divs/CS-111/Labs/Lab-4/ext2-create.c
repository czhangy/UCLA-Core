...
#define BLOCKS_PER_GROUP (NUM_BLOCKS * 8)
#define INF_COUNT -1
...
// Block group descriptor defines
#define NUM_DIRS 2

// Bitmap defines
#define FREE_BYTE 0
#define USED_BYTE 0xFF
#define NUM_WR_BYTES 1
#define NUM_BYTES 1024
#define FIRST_BLOCK_BITMAP_BOUND 2
#define SECOND_BLOCK_BITMAP_BOUND 127
#define FIRST_INODE_BITMAP_BOUND 1
#define SECOND_INODE_BITMAP_BOUND 15

// Inode table defines
#define ROOT_USER 0
#define NORMAL_USER 1000
#define DIR_SIZE 1024
#define NUM_I_BLOCKS 2
...
void write_superblock(int fd) {
	...
	superblock.s_max_mnt_count     = INF_COUNT; /* Make this unlimited */
...
void write_block_bitmap(int fd) {
	// Get offset of block bitmap
	off_t off = lseek(fd, BLOCK_OFFSET(BLOCK_BITMAP_BLOCKNO), SEEK_SET);
	if (off == -1) {
		errno_exit("lseek");
	}
	// Hard-code byte values
	u_int32_t used = USED_BYTE, free = FREE_BYTE, byte_2 = 0x7F, byte_127 = 0x80;
	// Set bitmap byte by byte
	for (u_int32_t byte_num = 0; byte_num < NUM_BYTES; byte_num++) {
		// Set blocks 1 -> 16 and 1024 -> 8192 to used
		if (byte_num < FIRST_BLOCK_BITMAP_BOUND || byte_num > SECOND_BLOCK_BITMAP_BOUND) {
			if (write(fd, &used, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		// Set blocks 17 -> 24 to appropriate bits
		} else if (byte_num == FIRST_BLOCK_BITMAP_BOUND) {
			if (write(fd, &byte_2, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		// Set blocks 1017 -> 1024 to appropriate bits
		} else if (byte_num == SECOND_BLOCK_BITMAP_BOUND) {
			if (write(fd, &byte_127, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		// Set blocks 25 -> 1016 to free
		} else {
			if (write(fd, &free, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		}
	}
}
...
void write_inode_bitmap(int fd) {
	...
	// Hard-code byte values
	u_int32_t used = USED_BYTE, free = FREE_BYTE, byte_1 = 0x1F;
	// Set bitmap byte by byte
	for (u_int32_t byte_num = 0; byte_num < NUM_BYTES; byte_num++) {
		// Set blocks 1 -> 8 and 129 -> 8192 to used
		if (byte_num < FIRST_INODE_BITMAP_BOUND || byte_num > SECOND_INODE_BITMAP_BOUND) {
			if (write(fd, &used, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		// Set blocks 9 -> 16 to appropriate bits
		} else if (byte_num == FIRST_INODE_BITMAP_BOUND) {
			if (write(fd, &byte_1, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		// Set blocks 17 -> 128 to free
		} else {
			if (write(fd, &free, NUM_WR_BYTES) != NUM_WR_BYTES)
				errno_exit("write");
		}
...
void write_inode_table(int fd) {
	...
	root_inode.i_blocks = NUM_I_BLOCKS;
	...
	lost_and_found_inode.i_mode = EXT2_S_IFDIR
	                              ...
	                              | EXT2_S_IROTH
	                              | EXT2_S_IXOTH;
	lost_and_found_inode.i_uid = ROOT_USER;
	lost_and_found_inode.i_size = DIR_SIZE;
	...
	lost_and_found_inode.i_gid = ROOT_USER;
	lost_and_found_inode.i_links_count = 2;
	lost_and_found_inode.i_blocks = NUM_I_BLOCKS; /* These are oddly 512 blocks */
	...
	hello_inode.i_gid = NORMAL_USER;
	hello_inode.i_links_count = 1;
	hello_inode.i_blocks = 0;

	// Memcpy to i_block
	char src[] = "hello-world";
	memcpy(hello_inode.i_block, &src, HELLO_SIZE);
	...
	hello_world_inode.i_mode = EXT2_S_IFREG
						...
						| EXT2_S_IRGRP
						| EXT2_S_IROTH;
	hello_world_inode.i_uid = NORMAL_USER;
...
void write_root_dir_block(int fd) {
	...
	// Set parent entry and decrement
	...
	// Set hello-world entry and decrement
	...
	// Set lost+found entry and decrement
...
void write_lost_and_found_dir_block(int fd) {
	// Get lost and found directory block's offset
...
void write_hello_world_file_block(int fd) {
	// Get hello world file block's offset
	off_t off = BLOCK_OFFSET(HELLO_WORLD_FILE_BLOCKNO);
	off = lseek(fd, off, SEEK_SET);
	if (off == -1)
		errno_exit("lseek");
	// Hard code contents of file
	char contents[] = "Hello world\n";
	// Write into file descriptor
	if (write(fd, &contents, HELLO_WORLD_SIZE) != HELLO_WORLD_SIZE)
		errno_exit("write");
}
