...
public:
  ...
  {
    m_router.processPacket(packet, inIface);
  }
...
public:
  ...
  run(int, char*[]) override
  {
    ...
    auto aclFile = communicator()->getProperties()->getPropertyWithDefault("ACLTable", "ATABLE");
    if (!m_router.loadACLTable(aclFile)) {
      std::cerr << "ERROR: Cannot load ACL table from `" << rtFile << "`" << std::endl;
      return EXIT_FAILURE;
    }
