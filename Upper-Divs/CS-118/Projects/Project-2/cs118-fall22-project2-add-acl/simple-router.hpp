...
public:
   ...
   */
  void
  processPacket(const Buffer& packet, const std::string& inIface);
  ...
  /**
   * Load ACL table information from \p aclConfig file
   */
  bool
  loadACLTable(const std::string& aclConfig);
...
inline const ACLTable&
SimpleRouter::getACLTable() const
{
  return m_aclTable;
}
